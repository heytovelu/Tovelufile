...
<svg width="28" height="28" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" class="shrink-0">
  <defs>
    <linearGradient id="tovelu-cyan-blue" x1="16" y1="16" x2="50" y2="84" gradientUnits="userSpaceOnUse">
      <stop offset="0%" stop-color="#00C0FF" />
      <stop offset="100%" stop-color="#0062FF" />
    </linearGradient>
    <linearGradient id="tovelu-mint-emerald" x1="84" y1="16" x2="50" y2="84" gradientUnits="userSpaceOnUse">
      <stop offset="0%" stop-color="#00F5A0" />
      <stop offset="100%" stop-color="#00D06C" />
    </linearGradient>
  </defs>
  <path d="M50 16 C28 16, 16 32, 16 50 C16 68, 28 84, 50 84 C38 72, 34 58, 34 50 C34 42, 38 28, 50 16 Z" fill="url(#tovelu-cyan-blue)" />
  <path d="M50 16 C72 16, 84 32, 84 50 C84 68, 72 84, 50 84 C62 72, 66 58, 66 50 C66 42, 62 28, 50 16 Z" fill="url(#tovelu-mint-emerald)" />
  <circle cx="50" cy="50" r="8.5" fill="#00F5A0" />
</svg>
`;

// Assemble the unified app HTML with official brand tokens and full interactivity
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Geist:wght@400;500;600;700&family=Ubuntu:wght@700&display=swap" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" rel="stylesheet">
  
  <!-- Tailwind CSS with Official Tovelu Palette & Stitch Design Tokens -->
...
<body>
  ...
  <div class="app-viewport">
    ...
    <header class="fixed top-0 max-w-[440px] w-full z-50 bg-surface/90 backdrop-blur-xl pt-safe shadow-[0_1px_8px_rgba(0,0,0,0.04)] border-b border-surface-container">
      <div class="h-14 px-gutter flex items-center justify-between">
        <div class="flex items-center gap-2 cursor-pointer" onclick="switchMainTab('flow')">
          ${toveluLogoSvg}
          <span class="text-on-surface font-brand text-[22px] tracking-tight font-bold">tovelu</span>
        </div>
    ...
    <nav class="fixed bottom-0 max-w-[440px] w-full z-50 pb-safe bg-surface/95 backdrop-blur-xl shadow-[0_-1px_8px_rgba(0,0,0,0.04)] border-t border-surface-container">
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      // 2. Hook Food Action Buttons: Scan, Manually, Ask AI
      ...
      promptButtons.forEach(b => {
        ...
        if (txt.includes('What 15m') || txt.includes('Why do I feel') || txt.includes('When is the best') || txt.includes('Can I drink tea')) {
          b.onclick = () => {
      ...
      // 5. Hook Chat Send Action
      const chatInput = document.querySelector('#pane-chat input');
      const chatSendBtn = document.querySelector('#pane-chat button span.material-symbols-outlined')?.parentElement;
      if (chatInput) {
      ...
      if (mode === 'scan') {
        ...
        body.innerHTML = \`
          <div class="border-2 border-dashed border-outline-variant/40 rounded-xl p-6 text-center flex flex-col items-center gap-3">
            ...
            <button class="bg-primary text-on-primary px-5 py-2.5 rounded-lg font-label-sm text-[12px] uppercase tracking-wider font-semibold shadow-sm hover:bg-primary/90" onclick="document.getElementById('camera-file-input').click()">
        ...
        body.innerHTML = \`
          <div class="flex flex-col gap-3">
            ...
            <button class="w-full bg-primary text-on-primary py-3 rounded-lg font-label-sm text-[12px] uppercase tracking-wider font-semibold shadow-sm hover:bg-primary/90" onclick="showScanResult()">
    ...
    function showScanResult() {
      ...
      out.innerHTML = \`
        <div class="p-4 rounded-xl bg-optimal-green/5 border border-optimal-green/30 flex flex-col gap-2 mt-2">
          ...
          <button class="w-full bg-primary text-on-primary py-2.5 rounded-lg font-label-sm text-[11px] uppercase tracking-wider font-bold mt-2 hover:bg-primary/90" onclick="alert('✓ 34g Protein & Calorie Target Logged!'); closeFoodModal();">
    ...
    // Live AI Chat Message Engine
    function sendAppChatMessage() {
      const input = document.querySelector('#pane-chat input');
      ...
      // Find the main greeting card in chat and insert conversation
      const greetingCard = document.querySelector('#pane-chat .bg-surface-container-lowest');
      if (greetingCard) {
        const userMsgDiv = document.createElement('div');
        userMsgDiv.className = 'p-3.5 bg-primary text-on-primary rounded-xl font-body-md text-[14px] self-end max-w-[85%] ml-auto mt-3 shadow-sm';
        userMsgDiv.innerText = userText;

        const aiMsgDiv = document.createElement('div');
        aiMsgDiv.className = 'p-4 bg-surface-container-low border border-primary/20 rounded-xl font-body-md text-[14px] text-on-surface mt-2 shadow-sm flex flex-col gap-2';
        aiMsgDiv.innerHTML = \`
          <div class="flex items-center gap-1 text-primary font-label-sm text-[11px] uppercase tracking-wider font-bold">
            <span class="material-symbols-outlined text-[16px]">auto_awesome</span>
            Tovelu Clinical Intelligence
          </div>
          <div>Based on your <strong>42-biomarker calibration</strong> (fasting glucose 84 mg/dL) and stocked pantry, here is your clinical answer: Ensure at least 30g whole protein with zero industrial seed oils to maintain flat glucose stability.</div>
        \`;

        greetingCard.appendChild(userMsgDiv);
        greetingCard.appendChild(aiMsgDiv);
